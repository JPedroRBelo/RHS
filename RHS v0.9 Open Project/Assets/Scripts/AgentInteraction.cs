...
public class AgentInteraction : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        crouchValue = anklePosition.position.y;
        ...
        {            
            for (int i = 0; i < agentHands.Length; i++)
            {
                if (agentHands[i].focus != null)
                {
                    ...
                    if(isReachable(agentHands[i]) && agentHands[i].ik)
                    {
                        animator.SetFloat(agentHands[i].getAnimatorParam(), 1, 0.1f, Time.deltaTime * 0.2f);
    ...
    void LateUpdate()
    {
        ...
        anklePosition.localPosition = Vector3.Lerp(anklePosition.localPosition, new Vector3(anklePosition.localPosition.x,
            crouchValue, anklePosition.localPosition.z), turningRate * Time.deltaTime);
    }
    ...
    private void manageHandPosition(Hand hand)
    {
        if (hand.isHandFree())
        {
            if (isReachable(hand))
            {
                ...
                //O foco está na posição
                if (isInPosition(hand.focus.position, hand.focusDesiredPosition) && animator.GetFloat(hand.getAnimatorParam()) > 0.9)
                {
                    //A mão está posicionada no foco? O IK já terminou de executar?
            ...
            else if (isInPosition(hand.focus.position, hand.getCommand().getNearestDesiredPosition(transform.position)))
            {
                ...
                Debug.Log("Command>>> "+this.name+" command " + hand.getCommand().getId() + " Failed! Location is not reachable.");
            }
        }
        else
        {
            Debug.Log("Command>>> "+this.name+" command " + hand.getCommand().getId() + " Failed! Hand is busy.");
            hand.getCommand().fail();
        }
    }
